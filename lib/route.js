...
class route {
	...
	router(path) {
		...
		const _route = new route(this._cerus, this._path + path);

		this._cerus.promise(event => {
			this._stack.push(new layer(path, this._path, event, {route: _route}));
		})
		.then((req, res) => {
			_route.handle(req, res);
		});
	...
	remove(path, {method} = {}) {
		...
		for(let i = 0; i < this._stack.length; i++) {
			const _layer = this._stack[i];

			if(_layer.path === path && (method === undefined || (_layer.method === method || _layer.method === "ALL"))) {
		...
		if(!changed) throw new Error("the path " + path + " wasn't found, so couldn't be removed");
	}
	 ...
	 */
	use() {
		return this.route("*", {priority: true});
	}
	 ...
	 * @example
	 * cerus.router().route("/home", {method: "GET"})
	 * .then(function(req, res, next) {
	 ...
	 * @param {String} (options.method = "ALL") The method the route will use.
	 * @param {Boolean} (options.priority = false) The priority of the route.
	 * @return {Promise} This function will return a promise.
	 * @function route
	 */
	route(path, {method = "ALL", priority = false} = {}) {
		if(!this.check(path)) {
			throw new Error("the argument path must be a valid url path");
		}
		...
		return this._cerus.promise(function(event) {
			if(priority) {
				...
				this._stack.splice(index, 0, new layer(path, this._path, event, {method, priority}));
			}
			else {
...
for(var i = 0; i < methods.length; i++) {
	...
	route.prototype[method.toLowerCase()] = function(path, options) {
		return this.route(path, Object.assign({}, {method}, options));
	};
}
